            ...
            for vf_num in {1..7}; do
                ...
                if [ -d "$vf_path" ]; then
                    ...
                    if [ -f "$vf_path/exec_quantum_ms" ]; then
                        echo 32 | sudo tee "$vf_path/exec_quantum_ms" > /dev/null
                        actual_quantum=$(cat "$vf_path/exec_quantum_ms" 2>/dev/null)
                        echo "      Set $vf_path/exec_quantum_ms to 32 (actual: $actual_quantum)"
                    fi
                    
                    # Set preempt_timeout_us
                    if [ -f "$vf_path/preempt_timeout_us" ]; then
                        echo 64000 | sudo tee "$vf_path/preempt_timeout_us" > /dev/null
                        actual_timeout=$(cat "$vf_path/preempt_timeout_us" 2>/dev/null)
                        echo "      Set $vf_path/preempt_timeout_us to 64000 (actual: $actual_timeout)"
                    fi
                else
